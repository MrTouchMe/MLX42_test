...
static void	add_frame(t_animation *a, t_sprite *s, sprite_slice slice, int mirrored)
{
	...
	for (int i = 0; i < slice.height - (slice.padding_y * 2); i++)
	{
		...
		{
			if (mirrored)
				mlx_put_pixel(frame, (slice.height - (slice.padding_y * 2)) - j - 1,
				i, mlx_get_pixel(s->sprite_img, slice.x + j + slice.padding_x, slice.y + i + slice.padding_y));
			else
				mlx_put_pixel(frame, j, i, mlx_get_pixel(s->sprite_img, slice.x + j
				+ slice.padding_x, slice.y + i + slice.padding_y));
		}
	}
